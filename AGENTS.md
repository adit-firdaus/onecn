...
The package is published to npm as `@coolaf/onecn` (scoped under the `coolaf` org). The unscoped name `onecn` was rejected by npm's similar-name protection (`once`, `open`), so the scope is required.
...
- Subpath exports are auto-generated by `scripts/build-subpaths.ts` during build. Each component gets its own subpath (e.g., `@coolaf/onecn/button`, `@coolaf/onecn/dialog`).
